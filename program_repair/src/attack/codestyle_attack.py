...
def transfer_code_style(source_code, lang, attack_type):
    ...
    if lang == 'c':
        ...
        if attack_type[-1] == 'global':
            assert 1 == 2
            # new_code = transfer_code_style_global_java(source_code, attack_type)
        elif attack_type[-1] == 'local':
            new_code = transfer_code_style_local_java(source_code, attack_type)
    else:
        assert 1 == 2
...
def transfer_code_style_local_java(source_code, attack_type):
    variable_list, comment_list = [], []
    try:
        ...
        for path, node in tree:
            if isinstance(node, javalang.tree.VariableDeclarator):
                variable_list.append(node.name)
            if isinstance(node, javalang.tree.FormalParameter):
                variable_list.append(node.name)
            if isinstance(node, javalang.tree.Assignment):
                if hasattr(node.expressionl, 'member'):
                    variable_list.append(node.expressionl.member)
        # function name
        variable_list.append(tree.name)
    except:
